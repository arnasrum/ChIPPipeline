...
rule trim_galore:
    ...
    output:
        temp(expand("results/trim_galore/{sample}{read}.fastq", read=["_1", "_2"] if sfs.is_paired_end() else [""], allow_missing=True))
    conda:
        "../envs/trim.yml"
    params:
...
rule cutadapt:
    input:
        lambda wildcards: get_trim_input(wildcards.sample,"resources/reads","fastq")
    output:
        temp(expand("results/cutadapt/{sample}{read}.fastq", read=["_1", "_2"] if sfs.is_paired_end() else[""], allow_missing=True))
    conda:
        "../envs/cutadapt.yml"
    params:
...
rule fastp:
    ...
    output:
        temp(expand("results/fastp/{sample}{read}.fastq", read=["_1", "_2"] if sfs.is_paired_end() else[""], allow_missing=True))
    conda:
        "../envs/trim.yml"
    params:
